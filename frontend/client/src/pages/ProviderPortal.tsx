                ...
                <div className="row-item" key={s.id}>
                  <div>
                    ...
                    <small style={{ display: "block", opacity: 0.65 }}>
                      ₹{Math.round(s.predictionInr).toLocaleString("en-IN")} · {s.clientEmail} · {new Date(s.createdAt).toLocaleTimeString()}
                    </small>
                  </div>
                  {s.status === "pending_verification" ? (
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button
                        ...
                        onClick={() => { setSubmissionId(s.id); setHolderWallet(""); toast.info("Proof attached", { description: "Scroll to the policy form below — wallet address required." }); }}
                      >
                        Use for policy
                      </button>
                    </div>
            ...
            <div className="flow-grid">
              <div className="flow-field"><label>Verified submission</label>
                <select value={submissionId} onChange={(e) => setSubmissionId(e.target.value)}>
                  ...
                  {verified.map((s) => <option key={s.id} value={s.id}>{`${s.id.slice(0, 10)}… · ₹${Math.round(s.predictionInr).toLocaleString("en-IN")}`}</option>)}
                </select>
              </div>
